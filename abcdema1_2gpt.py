...
def check_signal(df, symbol):
    ...
    # body candle dalam point
    body_points = abs(last["close"] - last["open"]) / info.point
    ...
    lower_close = last["close"] < prev["close"]

    if body_points < MIN_BODY_POINTS:
        return None

    buy_signal = bullish_candle and higher_close and last["ema_fast"] > last["ema_slow"]
    sell_signal = bearish_candle and lower_close and last["ema_fast"] < last["ema_slow"]

    if REQUIRE_CROSS:
        buy_signal = buy_signal and buy_cross
        sell_signal = sell_signal and sell_cross
    ...
    while True:
        try:
            ...
            print(
                ...
                f"TotalPos={total_positions}"
            )

            # =========================
            # BLOK ARAH BERLAWANAN
            # =========================
            if signal == "BUY" and len(sell_positions) > 0:
                print(f"[{datetime.now()}] Ada SELL terbuka, BUY diblok")
                time.sleep(CHECK_INTERVAL)
                continue

            if signal == "SELL" and len(buy_positions) > 0:
                print(f"[{datetime.now()}] Ada BUY terbuka, SELL diblok")
                time.sleep(CHECK_INTERVAL)
                continue

            # =========================
            # MAKSIMAL 3 POSISI RUNNING
            # =========================
            if signal == "BUY":
                if len(buy_positions) >= MAX_BUY_POSITIONS:
                    print(f"[{datetime.now()}] BUY skip: posisi BUY sudah maksimal")
                    time.sleep(CHECK_INTERVAL)
                    continue
                ...
                if is_far_enough(tick.ask, buy_positions, pip_size, MIN_DISTANCE_PIPS):
                    ...
                    print(f"[{datetime.now()}] BUY skip: terlalu dekat dengan entry sebelumnya")

            elif signal == "SELL":
                if len(sell_positions) >= MAX_SELL_POSITIONS:
                    print(f"[{datetime.now()}] SELL skip: posisi SELL sudah maksimal")
                    time.sleep(CHECK_INTERVAL)
                    continue
                ...
                if is_far_enough(tick.bid, sell_positions, pip_size, MIN_DISTANCE_PIPS):
                    ...
                    print(f"[{datetime.now()}] SELL skip: terlalu dekat dengan entry sebelumnya")
