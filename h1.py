    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print(f"Indikator -> MA21: {latest['MA_21']:.2f} | MA144: {latest['MA_144']:.2f} | ADX: {latest['ADX']:.2f} | RSI: {latest['RSI']:.2f}")
                ...
                kondisi_bullish = (latest['close'] > latest['MA_144']) and (latest['MA_21'] > latest['MA_144'])
                kondisi_bearish = (latest['close'] < latest['MA_144']) and (latest['MA_21'] < latest['MA_144'])
                ...
                if kondisi_bullish:
                    # Pullback: Harga turun mendekati MA 21 atau MA 144
                    near_ma21 = abs(latest['low'] - latest['MA_21']) <= margin_of_error
                    near_ma144 = abs(latest['low'] - latest['MA_144']) <= margin_of_error
                    
                    if (near_ma21 or near_ma144) and (latest['RSI'] < 30 or prev['RSI'] < 30):
                        print("🔥 Sinyal BUY Ditemukan (Trend Naik + Pullback + Oversold)!")
                        self.send_order(mt5.ORDER_TYPE_BUY, ask_price)
                        
                elif kondisi_bearish:
                    # Pullback: Harga naik mendekati MA 21 atau MA 144
                    near_ma21 = abs(latest['high'] - latest['MA_21']) <= margin_of_error
                    near_ma144 = abs(latest['high'] - latest['MA_144']) <= margin_of_error
                    
                    if (near_ma21 or near_ma144) and (latest['RSI'] > 70 or prev['RSI'] > 70):
