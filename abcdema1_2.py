...
# --- FIXED mode ---
# contoh:
# XAUUSD 100 points = 10.0 kalau digits broker 2
# XAUUSD 100 points = 1.00 kalau digits broker 3
FIXED_SL_POINTS = 150
FIXED_TP_POINTS = 250
...
# kalau ATR mode -> trailing pakai ATR
TRAILING_MODE = "ATR"   # "ATR" atau "FIXED"
...
def run_forward_test():
    ...
    print(f"  No opposite positions at same time: YES")
    ...
    print(f"  Trailing stop: {'ON' if USE_TRAILING_STOP else 'OFF'}")
    if USE_TRAILING_STOP:
        if TRAILING_MODE.upper() == "ATR":
            print(f"  Trailing ATR: {ATR_TRAILING_MULTIPLIER}x")
        else:
            print(f"  Trailing Fixed: {FIXED_TRAILING_POINTS} points")
    print("")
    ...
    while True:
        try:
            ...
            print(
                ...
                f"Spread={spread:.1f} | Posisi={len(positions)} | "
                f"EMA={ema_state} | ATR={last['atr']:.2f} | Balance={balance_str}"
            )

            # 1 candle cuma 1 entry
            can_entry_this_bar = (_last_entry_bar_time != current_bar_time)

            if signal == "BUY":
                if not has_sell and can_entry_this_bar:
                    ok = send_order(SYMBOL, LOTS, mt5.ORDER_TYPE_BUY, last["atr"], MAGIC_NUMBER)
                    if ok:
                        _last_entry_bar_time = current_bar_time

            elif signal == "SELL":
                if not has_buy and can_entry_this_bar:
