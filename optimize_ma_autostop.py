...
# ==============================================================================
# 1. KONFIGURASI PARAMETER STRATEGI (DENGAN HARD MARGIN CALL / AUTO-STOP)
# ==============================================================================
SYMBOL = "XAUUSDm"
LOT = 0.01
SL_BUFFER_POINTS = 50
...
# ==============================================================================
# 2. NUMBA ENGINE DENGAN AUTO-STOP SAAT MARGIN CALL (BALANCE <= 0)
# ==============================================================================
@nb.njit(parallel=True, fastmath=True)
def run_all_ma_combinations_autostop(
    ...
    # Kolom output: [ma_fast, ma_slow, final_balance, total_trades, win_trades, loss_trades, is_mc]
    ...
    for idx in nb.prange(total_combinations):
        ...
        for i in range(start_idx, n):
            # [PROTEKSI AUTO-STOP]: Jika Saldo Habis / MC -> LANGSUNG BERHENTI TRADING
            if balance <= 0.0:
                balance = 0.0
                is_mc = 1.0
                break
            ...
            # Cek kembali setelah SL
            if balance <= 0.0:
                balance = 0.0
                is_mc = 1.0
                break
            
            # B. Evaluasi Sinyal pada Candle Selesai (i-1)
            ...
            if signal != 0:
                ...
                # C. Reverse Close jika arah berbalik
...
# ==============================================================================
# 4. MAIN OPTIMIZATION RUNNER DENGAN FILTER STRICT MARGIN CALL
# ==============================================================================
def optimize_autostop():
    print("=" * 90)
    print(" " * 15 + "MA 1-100 OPTIMIZER DENGAN AUTO-STOP (REAL MC CUT OFF)")
    print(" " * 10 + "Kombinasi yang bangkrut (Saldo <= 0) akan langsung STOP trading")
    print("=" * 90)
    ...
    for tf_name, csv_path in DATA_PATHS.items():
        ...
        print(f"[-] Menjalankan 10,000 kombinasi MA (MA1: 1..100 x MA2: 1..100) dengan proteksi Auto-Stop...")
        
        t_sim_start = time.time()
        raw_results = run_all_ma_combinations_autostop(
        ...
        df_res = pd.DataFrame(raw_results, columns=[
            "MA_Fast", "MA_Slow", "Final_Balance", "Total_Trades", "Win_Trades", "Loss_Trades", "Is_MC"
        ])
        ...
        print(f"[-] Statistik: {survivor_count:,} kombinasi SELAMAT ({survivor_count/100:.1f}%) | {mc_count:,} kombinasi MC / Saldo $0 ({mc_count/100:.1f}%)")
        
        top20 = df_res_sorted.head(20).copy()
        top20.index = range(1, 21)
        top20.index.name = "Rank"
        summary_survivor[tf_name] = top20
        
        # Simpan CSV hasil Top 20 dan Seluruh Kombinasi
        output_top20_csv = f"top20_ma_{tf_name.lower()}_autostop.csv"
        output_all_csv = f"all_combinations_{tf_name.lower()}_autostop.csv"
        ...
        print(f"\n{'='*30} TOP 20 MA PARAMETERS (SURVIVOR) [{tf_name}] {'='*30}")
    ...
    # ==============================================================================
    # 5. REKAPITULASI JUARA 1 REAL TIAP TIMEFRAME
    # ==============================================================================
    print("\n" + "#" * 95)
    print(" " * 20 + "RINGKASAN PARAMETER TERBAIK (TIDAK PERNAH MC DARI MODAL $100)")
    print("#" * 95)
    for tf_name, top_df in summary_survivor.items():
        best = top_df.iloc[0]
