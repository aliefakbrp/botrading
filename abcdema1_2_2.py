...
# RSI EXIT
RSI_PERIOD = 14
RSI_SELL_CLOSE_LEVEL = 30
RSI_BUY_CLOSE_LEVEL = 70
USE_RSI_PROFIT_EXIT = True
...
def close_position(position):
    ...
    if info is None or tick is None:
        print(f"[-] Gagal close posisi {position.ticket}: tick/info tidak ada")
        return False
    ...
    print(f"[{datetime.now()}] Close retcode={result.retcode} ({retcode_text(result.retcode)}) | ticket={position.ticket}")

    if result.retcode == mt5.TRADE_RETCODE_DONE:
        print(f"[{datetime.now()}] [OK] {side} CLOSED by RSI profit exit | ticket={position.ticket}")
        return True

    print(f"[{datetime.now()}] [-] Gagal close {side} | ticket={position.ticket} | {retcode_text(result.retcode)}")
    return False

def update_rsi_profit_exit(positions, rsi_value):
    if not USE_RSI_PROFIT_EXIT:
        return

    for pos in positions:
        # hanya close kalau posisi sedang profit
        if pos.profit <= 0:
            continue

        if pos.type == mt5.POSITION_TYPE_SELL and rsi_value <= RSI_SELL_CLOSE_LEVEL:
            print(
                f"[rsi-exit] SELL ticket {pos.ticket} | "
                f"RSI={rsi_value:.2f} <= {RSI_SELL_CLOSE_LEVEL} | "
                f"Profit={pos.profit:.2f} -> close"
            )
            close_position(pos)

        elif pos.type == mt5.POSITION_TYPE_BUY and rsi_value >= RSI_BUY_CLOSE_LEVEL:
            print(
                f"[rsi-exit] BUY ticket {pos.ticket} | "
                f"RSI={rsi_value:.2f} >= {RSI_BUY_CLOSE_LEVEL} | "
                f"Profit={pos.profit:.2f} -> close"
            )
            close_position(pos)
...
def run_forward_test():
    ...
    print(f"  RSI exit SELL <= {RSI_SELL_CLOSE_LEVEL} (profit only)")
    print(f"  RSI exit BUY  >= {RSI_BUY_CLOSE_LEVEL} (profit only)")
    ...
    while True:
        try:
            ...
            if positions:
                update_rsi_profit_exit(positions, current_rsi)
            ...
            if signal == "BUY":
                if current_rsi < RSI_BUY_CLOSE_LEVEL:  # filter RSI
                    ...
                    print(f"[{datetime.now()}] Skip BUY - RSI terlalu tinggi ({current_rsi:.2f})")

            elif signal == "SELL":
                if current_rsi > RSI_SELL_CLOSE_LEVEL:  # filter RSI
                    ...
                    print(f"[{datetime.now()}] Skip SELL - RSI terlalu rendah ({current_rsi:.2f})")
