# EMA 21 M15
# Entry:
# - Harga di atas EMA21 = BUY
# - Harga di bawah EMA21 = SELL
#
# Close / Reverse:
# - Kalau posisi BUY lalu harga di bawah EMA21:
#   tunggu 10 detik, cek ulang.
#   Kalau masih di bawah EMA21 => close BUY lalu open SELL.
#
# - Kalau posisi SELL lalu harga di atas EMA21:
#   tunggu 10 detik, cek ulang.
#   Kalau masih di atas EMA21 => close SELL lalu open BUY.
#
# Re-entry:
# - Kalau sudah close / cut loss dan harga masih valid di atas/bawah EMA21,
#   boleh entry lagi sesuai arah EMA21.
...
def get_ema_state(df):
    """
    Logic:
    - BUY kalau close candle berjalan di atas EMA21
    - SELL kalau close candle berjalan di bawah EMA21
...
def get_entry_signal_by_ema21(ema_state):
    """
    Entry:
    - BUY kalau harga close di atas EMA21
    - SELL kalau harga close di bawah EMA21
    """

    if ema_state["price_above_ema21"]:
        return "BUY"

    if ema_state["price_below_ema21"]:
        return "SELL"
...
def get_fresh_ema_state():
    """
    Ambil data candle terbaru dan hitung ulang EMA.
...
def run_forward_test():
    ...
    print(f"\n[{datetime.now()}] Bot EMA21 Price M15 aktif")
    print(f"  Symbol: {SYMBOL}")
    ...
    print(f"  Entry BUY : Harga close di atas EMA21")
    print(f"  Entry SELL: Harga close di bawah EMA21")
        ...
        try:
            ...
            if cl_closed:
                ...
                if ema_after_cl is None:
                    print(f"[{datetime.now()}] Setelah CL, gagal ambil EMA terbaru.")
                    time.sleep(CHECK_INTERVAL)
                    continue
                ...
                if reentry_signal is None:
                    print(f"[{datetime.now()}] Setelah CL, harga pas di EMA21 / belum valid untuk re-entry.")
                    time.sleep(CHECK_INTERVAL)
                    continue

                print(f"[{datetime.now()}] Setelah CL, open ulang sesuai posisi harga terhadap EMA21: {reentry_signal}")
            ...
            if total_positions > 0:
                ...
                if close_signal is not None:
                    ...
                    # Ambil data terbaru setelah tunggu 10 detik
                    ...
                    if ok:
                        ...
                        if reentry_signal is not None:
                            print(f"[{datetime.now()}] Open kebalikannya setelah konfirmasi: {reentry_signal}")
                            open_by_signal(reentry_signal)
                        else:
                            print(f"[{datetime.now()}] Tidak re-entry. Harga pas di EMA21 / belum jelas.")
            ...
            # =========================
            # KALAU BELUM ADA POSISI, ENTRY SESUAI HARGA VS EMA21
            # =========================
            if total_positions == 0:
                entry_signal = get_entry_signal_by_ema21(ema_state)

                if entry_signal is not None:
                    print(f"[{datetime.now()}] Tidak ada posisi. Open {entry_signal} karena harga valid terhadap EMA21.")
                    open_by_signal(entry_signal)
                else:
                    print(f"[{datetime.now()}] Tidak ada posisi. Harga pas di EMA21 / belum ada signal entry.")
