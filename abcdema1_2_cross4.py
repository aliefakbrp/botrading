# gausah nunggu ada cross langsung entry + cut loss 20 pips + re-entry sesuai EMA setelah CL
...
# Delay kecil setelah close sebelum open lawan / entry ulang
...
def run_forward_test():
    global _last_cross_key

    initialize_mt5()

    tf_name = "M5" if TIMEFRAME == mt5.TIMEFRAME_M5 else "M1"

    print(f"\n[{datetime.now()}] Bot EMA Realtime Cross Reversal aktif")
    print(f"  Symbol: {SYMBOL}")
    print(f"  Timeframe: {tf_name}")
    print(f"  EMA Fast / Slow: {EMA_FAST}/{EMA_SLOW}")
    print(f"  Entry: langsung ikut arah EMA kalau belum ada posisi")
    print(f"  Reverse: langsung close posisi lama lalu open posisi baru saat cross lawan")
    print(f"  Cut Loss: {CUT_LOSS_PIPS} pips")
    print(f"  Setelah CL: langsung entry lagi sesuai arah EMA")
        ...
        try:
            ...
            if cl_closed:
                ...
                if current_direction is None:
                    print(f"[{datetime.now()}] EMA belum jelas setelah CL, tidak entry ulang.")
                    time.sleep(CHECK_INTERVAL)
                    continue

                print(f"[{datetime.now()}] Setelah CL, EMA sekarang {current_direction}. Langsung open {current_direction} lagi.")

                ok = open_by_signal(current_direction)

                if ok:
                    # reset supaya cross berikutnya tetap bisa kebaca normal
                    _last_cross_key = None
            ...
            # =========================
            # KALAU BELUM ADA POSISI, LANGSUNG IKUT ARAH EMA
            # =========================
            if total_positions == 0 and current_direction is not None:
                print(f"[{datetime.now()}] Tidak ada posisi. EMA sekarang {current_direction}. Langsung open {current_direction}.")
            ...
            # =========================
            # EKSEKUSI LANGSUNG SAAT ADA CROSS BARU
            # =========================
            if signal is not None and cross_key != _last_cross_key:
                print(f"[{datetime.now()}] REALTIME CROSS {signal} terdeteksi di candle {signal_bar_time}")

                positions = get_open_positions(SYMBOL, MAGIC_NUMBER)
                total_positions = len(positions)

                # Belum ada posisi -> langsung open sesuai signal
                if total_positions == 0:
                    print(f"[{datetime.now()}] Tidak ada posisi. Langsung open {signal}.")
                    ok = open_by_signal(signal)
                ...
                # Ada posisi -> cek apakah sama arah atau lawan arah
                current_position_type = None
                if positions:
                    first_pos = positions[0]
                    if first_pos.type == mt5.POSITION_TYPE_BUY:
                        current_position_type = "BUY"
                    else:
                        current_position_type = "SELL"

                # Kalau posisi sudah sama dengan signal, tidak usah entry ulang
                if current_position_type == signal:
                    print(f"[{datetime.now()}] Posisi sudah {signal}. Tidak open ulang.")
                    _last_cross_key = cross_key
                    time.sleep(CHECK_INTERVAL)
                    continue

                # Kalau posisi lawan arah, close semua lalu open sesuai signal
                print(f"[{datetime.now()}] Posisi lama {current_position_type}, signal baru {signal}.")
                print(f"[{datetime.now()}] Langsung close posisi lama lalu open {signal}.")
                ...
                if positions_after_close:
                    print(f"[{datetime.now()}] Masih ada posisi yang belum tertutup. Open {signal} dibatalkan.")
                    time.sleep(CHECK_INTERVAL)
                    continue
