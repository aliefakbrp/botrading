...
def check_cross_signal(df):
    ...
    if buy_cross:
        return "BUY", signal_bar_time
    elif sell_cross:
        return "SELL", signal_bar_time

    return None, signal_bar_time
...
def send_order(symbol, lot, order_type, magic):
    ...
    request = {
        ...
        "deviation": 20,
        "magic": magic,
        "comment": "EMA 1/2 CROSS REVERSAL",
...
def run_forward_test():
    global _last_cross_bar_time

    initialize_mt5()

    print(f"\n[{datetime.now()}] Bot EMA Cross Reversal aktif")
    print(f"  Symbol: {SYMBOL}")
    print(f"  Timeframe: M1")
    print(f"  EMA Fast / Slow: {EMA_FAST}/{EMA_SLOW}")
    print(f"  Entry: hanya saat CROSS")
    print(f"  Exit : close saat cross lawan")
    ...
    while True:
        try:
            ...
            signal, signal_bar_time = check_cross_signal(df)
            last_closed = df.iloc[-2]
            ema_state = "Bullish" if last_closed["ema_fast"] > last_closed["ema_slow"] else "Bearish"
            ...
            if signal is not None and signal_bar_time != _last_cross_bar_time:
                print(f"[{datetime.now()}] CROSS {signal} terdeteksi di candle {signal_bar_time}")
                ...
                if positions_after_close:
                    ...
                    if signal == "BUY":
                        ...
                        if ok:
                            _last_cross_bar_time = signal_bar_time
                    elif signal == "SELL":
