...
# =========================
# SIGNAL CROSS - REALTIME
# =========================
def check_cross_signal_realtime(df):
    if len(df) < 10:
        return None, None, None, None

    prev_candle = df.iloc[-2]     # candle sebelumnya, sudah close
    current_candle = df.iloc[-1]  # candle sekarang, masih jalan
...
def run_forward_test():
    ...
    print(f"  Entry: saat CROSS realtime + konfirmasi {CONFIRM_SECONDS} detik")
    print(f"  Exit : close langsung saat cross lawan")
    ...
    while True:
        try:
            ...
            ema_state = "Bullish" if current_candle["ema_fast"] > current_candle["ema_slow"] else "Bearish"
            ...
            if total_positions > 0 and tick is not None and info is not None:
                ...
                pos_text = "BUY" if is_buy else "SELL"
            ...
            if signal is not None and cross_key != _last_cross_key:
                ...
                print(f"[{datetime.now()}] Tunggu konfirmasi {CONFIRM_SECONDS} detik...")
                ...
                if not confirmed:
                    print(f"[{datetime.now()}] Sinyal {signal} batal, setelah {CONFIRM_SECONDS} detik EMA sudah berubah.")
                    continue

                print(
                    f"[{datetime.now()}] Sinyal {signal} valid setelah {CONFIRM_SECONDS} detik | "
                ...
                if positions:
                    print(f"[{datetime.now()}] Menutup posisi lama dulu...")
                ...
                if positions_after_close:
                    print(f"[{datetime.now()}] Masih ada posisi yang belum tertutup, open baru dibatalkan.")
                else:
